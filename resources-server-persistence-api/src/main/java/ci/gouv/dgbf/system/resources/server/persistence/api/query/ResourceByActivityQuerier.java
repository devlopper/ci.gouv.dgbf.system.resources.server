...
public interface ResourceByActivityQuerier extends ByDimensionOneBusinessIdentifierQuerier<Resource,Activity,String> {
	...
	String QUERY_NAME_READ_BY_ACTIVITIES_CODES = "readByActivitiesCodes";
	String QUERY_IDENTIFIER_READ_BY_ACTIVITIES_CODES = QueryIdentifierBuilder.getInstance().build(Resource.class, QUERY_NAME_READ_BY_ACTIVITIES_CODES);
	String QUERY_VALUE_READ_BY_ACTIVITIES_CODES_WHERE = " WHERE t.activity.code IN :"+PARAMETER_NAME_ACTIVITIES_CODES
			+" AND t.budget.actVersion.code = :"+PARAMETER_NAME_BUDGETARY_ACT_VERSION_CODE+" "
			;
	String QUERY_VALUE_READ_BY_ACTIVITIES_CODES = ResourceQuerier.QUERY_VALUE_READ_VIEW_01_SELECT_FROM+QUERY_VALUE_READ_BY_ACTIVITIES_CODES_WHERE
			+" ORDER BY t.activity.code ASC,t.economicNature.code ASC";
	
	String QUERY_NAME_COUNT_BY_ACTIVITIES_CODES = "countByActivitiesCodes";
	
	static void initialize() {
		QueryHelper.addQueries(Query.build(Query.FIELD_IDENTIFIER,QUERY_IDENTIFIER_READ_BY_ACTIVITIES_CODES
				,Query.FIELD_TUPLE_CLASS,Resource.class,Query.FIELD_RESULT_CLASS,Resource.class
				,Query.FIELD_VALUE,QUERY_VALUE_READ_BY_ACTIVITIES_CODES
